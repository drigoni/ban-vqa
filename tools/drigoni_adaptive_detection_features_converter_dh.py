...
def parse_args():
    ...
    parser.add_argument('--len_features', type=int, default=2048, help='len of features')
    args = parser.parse_args()
    return args

def extract(split, infiles, task='vqa', len_features=2048):
    ...
    for infile in infiles:
        ...
        with open(infile, "r+") as tsv_in_file:
            ...
            for item in reader:
                ...
                bboxes = np.frombuffer(base64.b64decode(item['boxes']), dtype=np.float64)
...
if __name__ == '__main__':
    ...
    if args.task == 'vqa':
        ...
        extract('train', infiles, args.task, len_features=args.len_features)
        extract('val', infiles, args.task, len_features=args.len_features)
        infiles = ['data/test2015/test2015_resnet101_faster_rcnn_genome.tsv']
        extract('test', infiles, args.task, len_features=args.len_features)
